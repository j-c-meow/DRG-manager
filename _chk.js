...
/* =========================================================
 * 深岩银河 · 17号钻台管理终端  v0.2
 * 粉丝非商用二创 | 单文件 | 数据驱动（CONFIG 可调）
 * 文案素材：官方本地化文本（资料/raw/*_文本全量_en_zh.tsv）
 * ========================================================= */
...
/* ---------------- 界面文案（B-2） ---------------- */
/* ============================================================
   TEXT 一键注入包 v1.3（B 维护，2026-09-13）
   来源：游戏文案包 v1.3(214) + 深潜系统设计 dd_*(32) + 精英支援位系统 el_*(12)
   用法：整个文件内容粘贴进 游戏.html <script> 内（TEXT 首次定义之前不能有其他 TEXT 声明）
   键数：258；已过 node --check 与键数/重复校验
   ============================================================ */
...
/* ============================================================
   ...
   来源：游戏文案包 v1.3(214) + 深潜系统设计 dd_*(32) + 精英支援位系统 el_*(12)
   用法：整个文件内容粘贴进 游戏.html <script> 内（TEXT 首次定义之前不能有其他 TEXT 声明）
   键数：311（主 267 + 深潜 dd 32 + 精英 el 12）；已过 node --check 与键数/重复校验
   ============================================================ */
...
/* ================= ev_lcyf_* / ev_cat_* 社区彩蛋事件（B 设计，2026-09-15） ================= */
/* 来源：资料\社区彩蛋事件设计.md §四（8 键）+ §四之二（9 键），共 17 键，逐字未动 */
Object.assign(TEXT, {
  ...
  inp.onchange = () => {
    ...
    r.onload = () => {
      try{
        ...
        if(d && d.v === 2){ S = d; log(TEXT.log_save_import_ok, 'good'); renderAll(); }
...
async function restoreFolderSave(){
  ...
  try{
    ...
    if(d && d.v === 2){ S = d; log(TEXT.log_restore_ok, 'good'); renderAll(); }
...
function log(msg, cls){
  S.log.unshift({t:clockStr(), m:msg, c:cls||''});
...
function genBoard(){ S.board = []; for(let i=0;i<6;i++) S.board.push(genMission()); S.boardAt = S.gm; }
...
function openDispatch(mid){
  ...
  const update = () => {
    ...
    const cost = dispatchCost(m, hc, modEff, dur);
...
function settle(d, offline, forceMul){
  ...
  if(S.autoUntil && Date.now() < S.autoUntil){
    ...
    (drops||[]).forEach(x => R.rare[x] = (R.rare[x]||0) + 1);
  ...
  if(!offline){
    ...
    (drops||[]).forEach(d=>rows.push(lootRowHTML('res_'+(MKEY[d]||'hollomite'), d, '+')));
    showLootToast(rows);
  }
  /* 卡尔彩蛋 */
...
function worldAdvanceBody(gm, offline){
  ...
  while(remain > 0 && guard < 200000){
    ...
    for(const d of S.deps){
      ...
      if(!offline) maybeEvent(d, d.done / d.dur);
    }
...
function renderDeps(){
  let html = '';
  if(!S.deps.length) html = '<div style="text-align:center;padding:8px 0;"><img src="assets/icons/misc_bosco.png" alt="" style="width:72px;image-rendering:pixelated;opacity:.8"><div class="note">'+TEXT.ui_deps_empty+'</div></div>';
  S.deps.forEach(d=>{
    /* 修复：进度与结算同源（d.done 累加），不再用 S.gm-start——事件挂起/加时不再显示背离 */
    const done = clamp(d.done || 0, 0, d.dur);
    const pct = (done/d.dur*100).toFixed(0);
...
function renderGear(){
  let html = '<div class="row"><button class="btn" onclick="showWarehouse()">📦 仓库</button><span class="note" style="flex:1">模组、饰品与空白模组的总仓库。</span></div>'+
    '<div class="note">每职业 3 级，每把武器 1 个模组槽。</div>';
  ...
  /* 锻造台：空白模组抽卡 */
  html += '<h3 class="sec">锻造台 · 模组抽卡</h3>'+animDiv('forge_card_flip', 64, 88)+'<div class="row"><span style="flex:1">空白模组：<b>'+(S.blanks||0)+'</b> 个</span>'+
    '<button class="btn pri" id="btn-draw" '+((S.blanks||0) >= 1 ? '' : 'disabled')+'>插入空白模组（3 选 1）</button></div>'+
    '<div class="note">完成任务板上的三提石任务（✦ 标记）获取空白模组。抽到重复模组自动折算 30% 造价的代币。</div>';
  ...
  Object.keys(CLASSES).forEach(c=>{
    ...
    [['main','主手池'],['off','副手池']].forEach(pair => {
      ...
      WEAPON_SET[c][pool].forEach((wid,i)=>{
        ...
        html += '<div class="row"><span style="flex:1;'+(carrying?'color:var(--amber);font-weight:bold;':(unlocked?'':'color:var(--dim);'))+'">'+
          '<img class="wicon" src="'+WICONS[c][pool][i]+'" style="image-rendering:pixelated;vertical-align:middle;margin-right:4px;">'+
          (carrying?'[携带中] ':'')+(unlocked?weaponZh(wid):'🔒 '+weaponZh(wid))+
          (unlocked?' <span class="note">Lv.'+lv+'/5</span>':'')+'</span>'+
          (unlocked && !carrying ? '<button class="btn" data-carryw="'+c+':'+pool+':'+i+'">携带</button>' : '')+
          (unlocked ? (lv<5 ? '<button class="btn" data-wup="'+c+':'+pool+':'+i+'" '+(canUp?'':'disabled')+'>'+(canUp?'升级':'缺 '+wupDisText(c,i))+'</button>' : '<span class="note">满级</span>') : '')+
          (unlocked && lv<5 ? '<div class="note" style="margin:-2px 0 4px 4px;">'+costHtml(uc)+(S.credits<uc.c?'（代币不足）':'')+'</div>' : '')+'</div>';
...
function ensureDiveWeek(){
  ...
  if(S.dive.week !== wk){
    ...
    log(TEXT['dd_expire_'+(1+Math.floor(Math.random()*2))], 'sys');
  }
}
...
function showResourceOverview(){
  ...
  TRADEABLES.forEach(t=>{
    const p = S.market.prices[t.k] || t.base;
    rows.push('<div class="row"><span style="flex:1">'+ic('res_'+MKEY[t.name])+' '+t.name+'</span><span>持有 <b>'+fmt(S[t.k]||0)+'</b>　市价 <b>'+p+'</b></span></div>');
  });
