...
function settle(d, offline, forceMul){
  ...
  if(d.mode === 'rush'){
    ...
    gain.credits = Math.round(gain.credits * (S.difficulty.yield||1));
  ...
  if(!offline){
    ...
    (drops||[]).forEach(d=>{ const nm=d.split('×')[0]; rows.push(lootRowHTML('res_'+(MKEY[nm]||'hollomite'), d, '+')); });
    showLootToast(rows);
  }
  /* 卡尔彩蛋 */
...
function renderBoard(){
  ...
  let html = '';
  /* B-9 序章剧情卡：卡尔的遗单（未完成序章时置顶显示） */
  if(!S.flags.prologueDone){
...
function playPrologue(){
  ...
  const go2 = () => { showModal(scr2, true);
    $('#pg-next').onclick = () => endPrologue();
    $('#pg-skip').onclick = () => endPrologue(); };
  $('#pg-next').onclick = go2;
  $('#pg-skip').onclick = () => endPrologue();
  ...
  window._spawnKarlMission = spawnKarlMission;
}
      ...
      ['main','off'].forEach(pool => {
        WEAPON_SET[cls][pool].forEach((wid,i)=>{
          ...
          body += '<div class="drow"'+(unlocked?'':' style="opacity:.45"')+'>'+
            ...
            '<span class="dvalue">'+(unlocked ? 'Lv.'+lv+'/5' : '未解锁')+'</span></div>';
        });
      });
    });
  } else if(tab === 2){
