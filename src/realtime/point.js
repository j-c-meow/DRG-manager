...
(function (root) {
  ...
  function PointBeacon(x, groundY, chunkYield) {
    ...
    this.rich = false;                          // 深度玩法：硬壳富矿（矿结采空后剩一层金壳）
    this.shellLeft = 0;
    this.t = Math.random() * 10;
  }

  /** 深度玩法：标记为硬壳富矿（由 mission 生成期调用） */
  PointBeacon.prototype.makeRich = function () {
    this.rich = true;
    this.shellLeft = 1;
  };

  /** 当前钻一层需要的时长：只剩硬壳时 ×2（深度玩法） */
  PointBeacon.prototype.drillNeed = function () {
    return DRILL_TIME * (this.chunksLeft <= 0 ? SHELL_MUL : 1);
  };
  ...
  PointBeacon.prototype.canDrill = function (p) {
    if (this.state !== 'active' || (this.chunksLeft <= 0 && this.shellLeft <= 0)) return false;
  ...
  PointBeacon.prototype.drill = function (dt, m) {
    ...
    if (Math.random() < dt * 26) {
      m.fx.burst(ex, ey, 2, {
        col: this.chunksLeft <= 0 ? ['#ffd76a', '#ffe9b0', '#c89a3a'] : ['#7fd4ff', '#c8ecff', '#4a9ad8'], speed: 190, life: 0.45, size: 2.6,
        ang: -2.6 + Math.random() * 1.2, spread: 0.9, kind: 1
      });
    }
    if (Math.random() < dt * 12) DRG.audio.sfx('pick', m.panOf(this.x), 0.7 + Math.random() * 0.2);
    m.shake(0.9, 0.05);
    if (this.drillT >= this.drillNeed()) {
      this.drillT = 0;
      if (this.chunksLeft <= 0 && this.shellLeft > 0) {
        /* 深度玩法：钻穿硬壳 → 富矿块（入库计 2） */
        this.shellLeft--;
        m.spawnChunk(this.x, this.y - 18, true);
        m.toast(L('硬壳破开了——里面是一整块富矿！'), '#ffd76a', 3);
        if (this.shellLeft <= 0) this.state = 'depleted';
      } else {
        this.chunksLeft--;
        m.spawnChunk(this.x, this.y - 18, false);
        if (this.chunksLeft <= 0) {
          if (this.shellLeft > 0) m.toast(L('矿结采空——硬壳里还封着富矿！'), '#ffd76a', 2.5);
          else { this.state = 'depleted'; m.toast(L('这处富矿点采完了'), '#9aa4b0', 2); }
        } else {
  ...
  PointBeacon.prototype.draw = function (g, cam) {
    ...
    // crystal cluster（数量随剩余矿块递减；只剩硬壳时画金色大晶体）
    var shellOnly = this.rich && this.shellLeft > 0 && this.chunksLeft <= 0;
    var crystals = shellOnly ? 1 : (active ? Math.max(1, Math.min(3, this.chunksLeft)) : 0);
    for (var i = 0; i < crystals; i++) {
      var ox = shellOnly ? 0 : (i - 1) * 14, h = shellOnly ? 52 : (i === 1 ? 42 : 30);
      var bob = Math.sin(t * 2.2 + i * 1.7) * 1.6;
      var golden = shellOnly;
      if (img) {
        gfx.sprite(g, img, sx + ox, sy - 12 - h / 2 + bob, h, { rot: (i - 1) * 0.14 });
        if (golden) gfx.spriteTint(g, img, sx + ox, sy - 12 - h / 2 + bob, h, '#ffd76a', 0.55, false);
      } else {
        g.save();
        g.fillStyle = golden ? '#ffd76a' : (i === 1 ? '#9adcff' : '#5fb2e8');
        g.beginPath();
    ...
    // drill progress ring（硬壳阶段金色 + 倍长进度）
    if (active && this.drillT > 0.04) {
      var need = this.drillNeed();
      var frac = M.clamp(this.drillT / need, 0, 1);
      var shellPhase = this.chunksLeft <= 0;
      gfx.ring(g, sx, sy - 62, 15, frac, shellPhase ? '#ffd76a' : '#7fd4ff', 4);
      gfx.text(g, Math.round(frac * 100) + '%', sx, sy - 84, { size: 12, align: 'center', col: shellPhase ? '#ffe9b0' : '#c8ecff' });
    }
    // approach hint
    if (active && this.drillT <= 0.04 && this.near) {
      var shellHint = this.chunksLeft <= 0;
      gfx.text(g, L(shellHint ? '按住左键 钻穿硬壳' : '按住左键 钻采矿结'), sx, sy - 66 + Math.sin(t * 5) * 2,
        { size: 12, align: 'center', col: gfx.pulse(t, shellHint ? '#ffd76a' : '#7fd4ff', '#ffffff', 6), alpha: 0.85 });
    }
  };
  ...
  /* ======================================================== 矿块（可携带实体） */
  function OreChunk(x, y, rich) {
    this.x = x; this.y = y;
    this.vx = (Math.random() - 0.5) * 50; this.vy = -90;
    this.state = 'idle';                        // idle | carried | spent
    this.t = 0; this.dead = false;
    this.kind = 'chunk';
    this.rich = !!rich;                         // 深度玩法：富矿块（入库计 2）
    this.label = this.rich ? '富矿块' : '矿块';
  ...
  OreChunk.prototype.draw = function (g, cam) {
    ...
    var sx = this.x - cam.x, sy = this.y - cam.y;
    var img = spr('aquarq');
    var glow = this.rich ? '#ffd76a' : '#7fd4ff';
    g.save();
    g.globalCompositeOperation = 'lighter';
    g.globalAlpha = 0.3 + 0.1 * Math.sin(this.t * 4);
    g.fillStyle = glow;
    g.beginPath(); g.arc(sx, sy, this.rich ? 19 : 15, 0, 6.283); g.fill();
    g.restore();
    if (img) {
      gfx.sprite(g, img, sx, sy - 4, this.rich ? 38 : 30, { rot: Math.sin(this.t * 1.8) * 0.2 });
      if (this.rich) gfx.spriteTint(g, img, sx, sy - 4, 38, '#ffd76a', 0.5, false);
    } else {
      g.save();
      g.fillStyle = this.rich ? '#e8b84a' : '#6fbde8';
      g.beginPath();
    ...
    if (this.state === 'idle' && this.pl && !this.pl.carriedItem &&
      ...
      gfx.text(g, 'E', sx, sy - 30, { size: 13, align: 'center', col: gfx.pulse(this.t, glow, '#ffffff', 5), alpha: 0.9 });
    }
  };
  OreChunk.prototype.lights = function (cam) {
    if (this.state !== 'spent') DRG.light.add(this.x - cam.x, this.y - cam.y - 8, this.rich ? 160 : 120, 0.55, this.rich ? [255, 215, 106] : [120, 205, 255], 0.4);
  };
